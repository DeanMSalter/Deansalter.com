...
async function deleteAllMessagesPerScreenFromDB(settings){
  const myConn = await globalConnection;
  return myConn.execute(
...
async function getSpecificBuildingNameFromDB(settings) {
  const myConn = await globalConnection;
  const [rows] = await myConn.execute(
...
async function deleteAllBuildingsFromDB(){
  const myConn = await globalConnection;
  let [rows] = await myConn.execute(
    'TRUNCATE buildings');
}
async function deleteSpecificBuildingFromDB(settings) {
  const myConn = await globalConnection;
  return myConn.execute(
    'DELETE FROM buildings WHERE uniqueID=?',
    [settings]);

}
async function getSpecificBuildingIDFromDB(settings) {
  const myConn = await globalConnection;
  const [rows] = await myConn.execute(
    'SELECT * FROM buildings WHERE uniqueID=?',[settings[0]]);
  return rows;
}
async function updateBuildingInDB(settings){
  const myConn = await globalConnection;
  return myConn.execute(
    'UPDATE buildings SET building = ? WHERE uniqueID=?',
...
async function deleteAllMessagesPerBuildingFromDB(settings){
  const myConn = await globalConnection;
  return myConn.execute(
    'DELETE FROM buildingMessages WHERE id=?',
    [settings]);
}
...
io.on('connection', function(socket) {
  ...
  async function updateMessageScreen(screenID,buildingID){
    ...
    if(!isEmpty(buildingID)){
      ...
      for(let o=0;o<buildings.length;o++){
        if(buildingID == buildings[o].uniqueID){
      ...
      for(let n=0;n<allSettings.length;n++){
        if(allSettings[0].building == buildingID){
            for(let p=0;p<socketIDs.length;p++){
                ...
                io.to(socketIDs[allSettings[0].id][p]).emit("updateLocation",building)
            }
  ...
  async function updateBackEndScreens(){
    ...
    io.emit("updateAllMessagesList",messages)
    io.emit("updateBuildingList",buildings)
    io.emit("updateAllBuildingMessages",buildingMessages)
  ...
  socket.on("deleteSetting",async function(data){
    ...
    await deleteAllMessagesPerScreenFromDB(data.id)
    updateBackEndScreens()
  })
  ...
  socket.on("saveBuilding",async function(data){
    console.log(data)
    if(!isEmpty(data.uniqueID)){
      let savedBuilding = await getSpecificBuildingIDFromDB(data.uniqueID)
      if(!isEmpty(savedBuilding)){
          await updateBuildingInDB([data.uniqueID,data.building])
          updateBackEndScreens()
          updateMessageScreen(null,data.uniqueID)
      }else{
          console.log(savedBuilding)
          await saveBuildingInDB()
          updateBackEndScreens()
          updateMessageScreen(null,data.uniqueID)
      }
    }else{
      console.log(data.building)
      let savedBuildingName = await getSpecificBuildingNameFromDB(data.building)
      if(!isEmpty(savedBuildingName)){
        socket.emit("duplicateBuilding",true)
      }else{
        await saveBuildingInDB(data.building)
  ...
  socket.on("purgeBuildings",async function (){
    await deleteAllBuildingsFromDB()
    updateBackEndScreens()
  })
  socket.on("deleteBuilding",async function (data){
    await deleteSpecificBuildingFromDB(data.uniqueID)
    await deleteAllMessagesPerBuildingFromDB(data.uniqueID)
    updateBackEndScreens()
    updateMessageScreen(null,data.uniqueID)
  })
  ...
  socket.on("saveBuildingMessage",async function(data){
    console.log(data)
    let savedBuilding = await getSpecificBuildingIDFromDB(data.buildingID)
    console.log(savedBuilding)
    if(isEmpty(savedBuilding)){
      socket.emit("BuildingSaved",false)
      return
    }else{
      socket.emit("BuildingSaved",true)
    }
      if(!isEmpty(data.uniqueID)){
